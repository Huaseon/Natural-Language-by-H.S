...
PHASES = [
    {  # Phase 1: 输出层
        'name': 'phase1', 'epochs': 18,
        'lrs': {
            'outputs': 1e-6,
        },
    },
    {  # Phase 2: + [CLS]池化层
        'name': 'phase2', 'epochs': 18,
        'lrs': {
            'outputs': 1e-6,
            'pooler': 8e-7,
        },
...
def split_train_val_test(df: pd.DataFrame, tokenizer: BertTokenizer) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    return random_split(dataset, [3/18, 2/18, 13/18])
